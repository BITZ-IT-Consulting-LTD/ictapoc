<template>
  <div class="o-page">
    <div v-if="service" class="u-animate-slide">
      <!-- Glass Header -->
      <div class="u-glass p-8 rounded-2xl mb-8 shadow-xl border-indigo-100 flex flex-col md:flex-row justify-between items-center gap-6">
        <div class="text-left">
        ...
        <div class="c-card__body p-10">
          <form @submit.prevent="handleSubmit" class="space-y-8">
              ...
              <div class="space-y-6">
                ...
                <div v-for="key in currentStep.fields" :key="key" class="c-field">
                  <label :for="key" class="c-field__label flex items-center justify-between">
                    ...
                    <span v-if="isRequired(key)" class="text-[10px] text-rose-500 font-bold bg-rose-50 px-1.5 py-0.5 rounded">REQUIRED</span>
                  </label>
                  
                  <!-- Select / Dropdown -->
                  <select 
                    v-if="schema.properties[key].enum" 
                    :id="key" 
                  ...
                  <!-- File Upload -->
                  <label v-else-if="schema.properties[key].format === 'data-url'" class="c-file-upload group h-32 flex flex-col justify-center items-center bg-indigo-50/20 border-indigo-200 border-2 border-dashed rounded-xl hover:bg-indigo-50 hover:border-indigo-400 transition-all cursor-pointer">
                    ...
                    <span class="text-indigo-600 font-bold text-sm">{{ formData[key]?.name || 'Click or Drag document here' }}</span>
                  </label>

                  <!-- Textarea -->
                  <textarea 
                    v-else-if="schema.properties[key].format === 'textarea'"
                    :id="key"
                    v-model="formData[key]"
                  ...
                  <div v-else-if="schema.properties[key].widget === 'radio'" class="flex gap-4 p-2">
                    <label v-for="opt in schema.properties[key].enum" :key="opt" class="flex items-center gap-2 cursor-pointer">
                      <input type="radio" :name="key" :value="opt" v-model="formData[key]" class="w-4 h-4 accent-indigo-600">
                      <span class="text-sm text-slate-600">{{ opt }}</span>
                    </label>
                  </div>

                  <!-- Checkbox Group (Multi-select) -->
                  <div v-else-if="schema.properties[key].widget === 'checkbox-group'" class="grid grid-cols-2 gap-3 p-2">
                    <label v-for="opt in (schema.properties[key].items?.enum || schema.properties[key].enum)" :key="opt" class="flex items-center gap-2 cursor-pointer p-3 bg-slate-50 border border-slate-200 rounded-lg hover:bg-white transition-all">
                      <input type="checkbox" :value="opt" v-model="formData[key]" class="w-4 h-4 accent-indigo-600 rounded">
                      <span class="text-xs font-medium text-slate-700">{{ opt }}</span>
                    </label>
                  </div>

                  <!-- Boolean Checkbox (Single) -->
                  <label v-else-if="schema.properties[key].type === 'boolean'" class="flex items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100">
                    <input type="checkbox" class="w-5 h-5 accent-indigo-600" v-model="formData[key]" :required="isRequired(key)">
            ...
            <div class="flex items-center justify-between pt-10 border-t border-slate-100 mt-10">
              ...
              <button 
                v-if="currentStepIndex < formSteps.length - 1"
                type="button"
                @click="handleNext"
...
const initializeForm = () => {
  ...
  Object.keys(schema.value.properties).forEach(key => {
    ...
    if (field.widget === 'checkbox-group') {
      ...
      // Intelligent Prefill
      let prefill = '';
      if (key.includes('id_number') || key === 'mother_id' || key === 'primary_owner_id') prefill = user.id_number || '';
      ...
      else if (key.includes('owner_name') || key === 'full_name') prefill = user.full_name || user.username || '';
      ...
      if (prefill && field.lookup_service) {
...
:deep(.c-input), :deep(.c-select), :deep(.c-editor) {
...
:deep(.c-input:focus), :deep(.c-select:focus), :deep(.c-editor:focus) {
