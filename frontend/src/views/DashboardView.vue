  ...
  <div>
    ...
    <div v-if="user">
      <div class="flex justify-between items-center mb-6">
        ...
        <router-link to="/profile" class="text-indigo-600 hover:text-indigo-800 font-semibold border border-indigo-200 px-4 py-2 rounded hover:bg-indigo-50">
          Manage Profile / Documents
        </router-link>
      </div>
      
      <!-- Citizen, Officer, Supervisor Common View -->
      <div v-if="['citizen', 'officer', 'supervisor', 'registrar', 'mda_admin'].includes(user.role)" class="space-y-8">
        <!-- Citizen Wallet Snapshot -->
        <div v-if="user.role === 'citizen' && user.saved_documents?.length" class="bg-indigo-900 text-white p-6 rounded-2xl shadow-xl overflow-hidden relative">
          ...
          <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              ...
              <p class="text-indigo-200 text-sm">You have {{ user.saved_documents.length }} authoritative documents issued.</p>
            </div>
            <div class="flex -space-x-3">
              <div v-for="(doc, i) in user.saved_documents.slice(0, 3)" :key="i" 
                ...
                <svg v-if="doc.type === 'AUTHORITATIVE_OUTPUT'" class="w-6 h-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                <svg v-else class="w-6 h-6 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              </div>
            </div>
            <router-link to="/profile" class="px-6 py-2 bg-white text-indigo-900 rounded-xl font-bold hover:bg-indigo-50 transition-colors shadow-lg shadow-indigo-950/20">
              Open Wallet
            </router-link>
          </div>
        </div>
        ...
        <div>
          <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            <h2 class="text-2xl font-semibold">Available Services</h2>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="service in filteredAvailableServices" :key="service.id" class="bg-white p-4 rounded-lg shadow">
              <h3 class="font-bold">{{ service.service_name }}</h3>
              <p class="text-sm text-gray-600">{{ getMdaName(service.mda) }}</p>
              <router-link :to="`/apply/${service.service_code}`" class="mt-4 inline-block text-indigo-600 hover:text-indigo-900 font-semibold">Apply Now &rarr;</router-link>
            </div>
          </div>
        </div>
        <div>
          <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            <h2 class="text-2xl font-semibold">Service Requests</h2>
            <div class="flex gap-4 max-w-xl w-full">
              ...
              >
              <select v-model="myRequestsStatusFilter" class="w-40 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          ...
          <div class="bg-white p-4 rounded-lg shadow">
             <p v-if="filteredMyRequests.length === 0" class="text-gray-500">No requests found.</p>
            <ul>
              <li v-for="request in filteredMyRequests" :key="request.id" class="border-b hover:bg-gray-50 transition-colors">
                <router-link :to="`/service-request/${request.id}`" class="block p-4 flex justify-between items-center">
                  <div>
                    <span class="font-semibold text-lg text-gray-800">{{ request.service_config.service_name }}</span>
                    <span class="text-sm text-gray-500 ml-2 block sm:inline">ID: {{ request.request_id }}</span>
                  </div>
                  <span class="px-3 py-1 text-xs font-semibold rounded-full uppercase tracking-wide" :class="statusClass(request.status)">
                    {{ request.status }}
                  </span>
                </router-link>
              </li>
      ...
      <div v-if="['officer', 'supervisor', 'registrar', 'mda_admin'].includes(user.role)" class="space-y-8 mt-8">
        ...
        <div class="bg-white border border-indigo-200 p-6 rounded-xl shadow-md">
          ...
          <div class="space-y-4">
            <div v-for="request in filteredAssignedRequests" :key="request.id" 
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span class="text-xs font-mono bg-white px-2 py-0.5 rounded border border-gray-200">{{ request.request_id }}</span>
                </div>
                <div class="text-sm text-gray-600 flex flex-wrap gap-x-4">
                  ...
                  <span>Status: <span class="px-2 py-0.5 text-[10px] font-bold rounded-full uppercase tracking-wider" :class="statusClass(request.status)">{{ request.status }}</span></span>
                </div>
              </div>
            ...
            <div v-if="filteredAssignedRequests.length === 0" class="py-12 text-center bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            ...
            <div class="flex gap-4 max-w-sm w-full">
              <input 
                ...
                placeholder="Find tasks in pool..." 
          ...
          <div class="bg-white overflow-hidden rounded-lg border border-slate-200 shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="request in filteredUnassignedRequests" :key="request.id" class="hover:bg-slate-50 transition-colors">
                  <td class="px-4 py-3">
                  ...
                  <td class="px-4 py-3 text-right">
                    <div class="flex justify-end gap-2">
                      <router-link :to="`/service-request/${request.id}`" class="text-slate-400 hover:text-slate-600 text-sm">Preview</router-link>
        ...
        <div class="bg-gray-50 border border-gray-200 p-6 rounded-xl shadow-sm opacity-90 transition-opacity hover:opacity-100">
          <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            <div>
            ...
            <table class="min-w-full divide-y divide-gray-100">
              ...
              <tbody class="divide-y divide-gray-50">
                <tr v-for="request in filteredIncompleteRequests" :key="request.id" class="hover:bg-gray-50/50">
                  ...
                  <td class="px-4 py-2">
                    <span class="px-2 py-0.5 text-[9px] font-bold rounded-full uppercase tracking-tighter" :class="statusClass(request.status)">
                      {{ request.status }}
                    </span>
                  </td>
                  <td class="px-4 py-2 text-right">
                    <router-link :to="`/service-request/${request.id}`" class="text-indigo-400 hover:text-indigo-600">&rarr;</router-link>
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div v-if="user.role === 'admin'" class="space-y-8">
        <div>
          ...
          <div class="mt-8">
            <div v-if="currentTab === 'MDAs'"><MdaManager /></div>
            <div v-if="currentTab === 'Services'"><ServiceConfigManager /></div>
            <div v-if="currentTab === 'Users'"><UserManager /></div>
            <div v-if="currentTab === 'Reports'"><ReportsDashboard /></div>
            <div v-if="currentTab === 'Roles'"><AdminRolesView /></div>
            <div v-if="currentTab === 'API Docs'"><ApiRegistry /></div>
            <div v-if="currentTab === 'Audit Logs'"><AuditLogManager /></div>
            <div v-if="currentTab === 'System Health'"><SystemHealthView /></div>
            <div v-if="currentTab === 'Security & Trust'"><SecurityTrustView /></div>
            <div v-if="currentTab === 'Architecture Pilot'"><ArchitectureSimulator /></div>
    ...
    <!-- Complete Step Modal -->
    <Teleport to="body">
      <div v-if="showCompleteStepModal" class="fixed inset-0 z-[9999] overflow-y-auto h-full w-full flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
        <div class="relative mx-auto p-8 border w-full max-w-md shadow-2xl rounded-2xl bg-white transform transition-all scale-100">
          <div class="mb-6 text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 bg-indigo-50 text-indigo-600 rounded-full mb-4">
              <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 002-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
            </div>
          ...
          <form @submit.prevent="completeStep" class="space-y-5">
            <div>
              <label for="step-action" class="block text-sm font-bold text-gray-700 mb-1">Taxonomy Status (Action)</label>
              <select v-model="stepAction.action" id="step-action" 
                      class="mt-1 block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none" required>
            ...
            <div>
              <label for="step-details" class="block text-sm font-bold text-gray-700 mb-1">Officer's Notes (Optional)</label>
