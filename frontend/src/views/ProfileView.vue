<template>
  <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 relative">
    <!-- Document View Modal -->
    <Teleport to="body">
      <div v-if="selectedPreviewDoc" class="fixed inset-0 z-[9999] overflow-y-auto flex items-center justify-center bg-gray-900/80 p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
          <div class="p-6 border-b flex justify-between items-center bg-slate-50">
            <div>
              ...
              <p class="text-xs text-slate-500 font-mono" v-if="selectedPreviewDoc.authoritative_id">Auth ID: {{ selectedPreviewDoc.authoritative_id }}</p>
            </div>
          ...
          <div class="flex-1 overflow-y-auto p-4 bg-slate-100 flex justify-center">
            ...
            <div v-if="selectedPreviewDoc.type === 'AUTHORITATIVE_OUTPUT'" class="bg-white w-[595px] min-h-[842px] shadow-lg p-12 relative border-8 border-double border-indigo-100 scale-90 sm:scale-100 origin-top">
               <!-- Ornamental Border -->
               <div class="absolute inset-4 border-2 border-indigo-50 pointer-events-none"></div>
               
               <div class="text-center mb-12">
                  <div class="flex justify-center mb-4">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/Coat_of_arms_of_Kenya_%28Official%29.svg/1200px-Coat_of_arms_of_Kenya_%28Official%29.svg.png" class="h-24 grayscale opacity-80" alt="GOK Coat of Arms">
                  </div>
                  <h1 class="text-2xl font-serif font-bold text-gray-900 uppercase tracking-widest">{{ selectedPreviewDoc.issued_by }}</h1>
               ...
               <div class="space-y-8 text-slate-800">
                 ...
                 <div class="pt-20 flex justify-between items-end">
                   <div>
                     ...
                     <p class="text-xs font-mono text-indigo-600">{{ selectedPreviewDoc.metadata?.digital_signature }}</p>
                   </div>
                   <!-- Mock Barcode -->
                   <div class="text-right">
                      <div class="bg-gray-900 w-32 h-10 flex gap-1 p-1 px-2 items-center justify-center">
                        <div class="bg-white w-0.5 h-full" v-for="i in 15" :key="i" :style="{ width: Math.random() * 3 + 'px' }"></div>
                      </div>
            ...
            <!-- Default File Viewer -->
            <iframe v-else :src="selectedPreviewDoc.content" class="w-full h-full min-h-[600px] bg-white rounded shadow-inner"></iframe>
          </div>
          <div class="p-4 bg-white border-t flex justify-end gap-3">
            <button @click="selectedPreviewDoc = null" class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium transition-colors">Close</button>
            <a v-if="selectedPreviewDoc.content" :href="selectedPreviewDoc.content" :download="selectedPreviewDoc.name" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium transition-colors">Download Copies</a>
          </div>
        </div>
      </div>
    </Teleport>
        ...
        <form @submit.prevent="updateProfile">
          <div class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-3">
              ...
              <input type="text" :value="user.username" disabled class="mt-1 block w-full bg-gray-100 border border-gray-300 rounded-md shadow-sm p-2">
            </div>

            <div class="col-span-6 sm:col-span-3">
              <label class="block text-sm font-medium text-gray-700">Email</label>
              <input type="email" v-model="form.email" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
            </div>

            <div class="col-span-6 sm:col-span-3">
              <label class="block text-sm font-medium text-gray-700">National ID Number</label>
              <input type="text" v-model="form.id_number" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
            </div>

            <div class="col-span-6 sm:col-span-3">
              <label class="block text-sm font-medium text-gray-700">Passport Number</label>
              <input type="text" v-model="form.passport_number" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
            </div>

            <div class="col-span-6 sm:col-span-3">
              <label class="block text-sm font-medium text-gray-700">Phone Number</label>
              <input type="tel" v-model="form.phone_number" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
            </div>

            <div class="col-span-6">
               <label class="block text-sm font-medium text-gray-700">Supporting Document for Verification (Required)</label>
               <input type="file" @change="handleVerificationDoc" class="mt-1 block w-full text-sm text-gray-500" accept=".pdf,.png,.jpg">
               <p class="text-xs text-gray-500 mt-1">Please attach ID or Passport scan to verify these changes.</p>
            </div>
          </div>
          <div class="mt-6 text-right">
             <button type="submit" class="bg-indigo-600 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
        <div>
           ...
           <p class="mt-1 max-w-2xl text-sm text-gray-500">Securely store documents (ID Scans, Certificates) for reuse.</p>
        </div>
        <button @click="showUpload = !showUpload" class="text-sm text-indigo-600 hover:text-indigo-900">+ Add Document</button>
      </div>
      ...
      <div v-if="showUpload" class="border-t border-gray-200 px-4 py-5 sm:p-6 bg-gray-50">
         <div class="grid grid-cols-1 gap-4">
            ...
            <input type="text" v-model="newDocName" placeholder="e.g. National ID Front" class="block w-full border border-gray-300 rounded-md p-2">
            ...
            <button @click="uploadDocument" :disabled="!newDocFile || !newDocName" class="mt-2 w-full bg-green-600 text-white py-2 rounded disabled:bg-gray-300">
               Upload to Wallet
            </button>
         </div>
      </div>
      ...
      <div class="border-t border-gray-200">
        <ul class="divide-y divide-gray-200">
           <li v-for="(doc, index) in savedDocs" :key="index" class="px-4 py-4 sm:px-6 flex justify-between items-center group">
              <div class="flex items-center space-x-3 overflow-hidden">
                 <div :class="[doc.type === 'AUTHORITATIVE_OUTPUT' || doc.doctype === 'BIRTH_CERTIFICATE' ? 'bg-indigo-100' : 'bg-gray-100', 'p-2 rounded-lg']">
                   <svg v-if="doc.type === 'AUTHORITATIVE_OUTPUT' || doc.doctype === 'BIRTH_CERTIFICATE'" class="h-6 w-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                 ...
                 <div class="flex flex-col truncate">
                   <div class="flex items-center space-x-2">
                     ...
                     <span v-if="doc.type === 'AUTHORITATIVE_OUTPUT' || doc.doctype === 'BIRTH_CERTIFICATE'" class="bg-blue-100 text-blue-700 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wide">Verified</span>
                   </div>
                   <div class="flex items-center gap-2" v-if="doc.authoritative_id">
                      <span class="text-xs text-gray-500 font-mono bg-gray-50 px-1 rounded border border-gray-200">ID: {{ doc.authoritative_id }}</span>
                   </div>
                   <span v-else class="text-xs text-gray-400">{{ (doc.size / 1024).toFixed(1) }} KB</span>
                 </div>
              </div>
              <div class="flex items-center space-x-4">
                <button v-if="doc.content || doc.doc_id" @click="viewDoc(doc)" class="text-indigo-600 hover:text-indigo-900 text-sm font-semibold">View</button>
                <button v-if="doc.type !== 'AUTHORITATIVE_OUTPUT' && doc.doctype !== 'BIRTH_CERTIFICATE'" @click="removeDoc(index)" class="text-red-400 hover:text-red-700 text-sm opacity-0 group-hover:opacity-100 transition-opacity">Remove</button>
              </div>
           </li>
