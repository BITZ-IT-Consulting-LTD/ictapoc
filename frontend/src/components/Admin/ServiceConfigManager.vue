<template>
  <div class="space-y-6 mt-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow">
      <h3 class="text-xl font-semibold mb-4 text-gray-900 border-b pb-2">National Service Registry</h3>

      <!-- Actions -->
      <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex flex-1 gap-4">
          <div class="relative max-w-sm flex-1">
            <input type="text" v-model="searchQuery" placeholder="Filter by name/code..."
              class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          </div>
          <select v-model="mdaFilter"
      ...
      <div v-if="showModal"
        ...
        <div class="relative mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white">
          <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900 text-center">{{ editForm.id ? 'Edit Service' :
              'Create New Service' }}</h3>
            <form @submit.prevent="handleUpdate" class="mt-2 text-left max-h-[80vh] overflow-y-auto">
              ...
              <div v-else class="mt-8 p-4 bg-yellow-50 text-yellow-800 rounded-md border border-yellow-200">
                <p class="text-sm font-medium">Please save the service details first before configuring workflow steps.
                </p>
              </div>
