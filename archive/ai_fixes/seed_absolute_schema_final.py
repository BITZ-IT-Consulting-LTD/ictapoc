...
updated = 0
all_services = ServiceConfig.objects.all()

for item in data:
    process_name = item["process_name"]
    
    # Brute-force match the service that actually has matching words in its name.
    words = process_name.split()
    if not words: continue

    service = None
    for svc in all_services:
        if sum(1 for w in words if w.lower() in svc.service_name.lower()) >= len(words) // 2 or len(words) < 2:
             if len(words) >= 2 and sum(1 for w in words if w.lower() in svc.service_name.lower()) == 0:
                 continue
             match = True
    ...
    if service:
        ...
        service.save()
        updated += 1
        print(f" ✓ Mapped JSON Form Schema automatically to: {service.service_name}")
    else:
        print(f" ✗ Could not find Service created from: {item['filename']}")
