...
print("=== SEEDING DETAILED INPUT SCHEMAS FOR PRIORITY MDAS ===")
...
for item in data:
    ...
    # We want to match explicitly what we just ran to fetch ALL services!
    import re
    # Match the explicit substring to ensure we map to the exact same Services.
    ...
    if service:
        # Generate custom properties based on process name
        properties = {
        ...
        elif "Enrollment" in process_name or "Student" in process_name:
        ...
        elif "Correspondence" in process_name:
        ...
        service.save()
        updated += 1
        print(f" ✓ Added Detailed Form Schema to: {service.service_name}")
