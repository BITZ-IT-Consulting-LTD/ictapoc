...
zip_filename = '/Users/mac/ictapoc/mdas/mda_bpm_docs_final.zip'
...
try:
    ...
    print(f"Generating Final Documentation for {len(items)} MDAs...")
    ...
    for item in items:
        ...
        to_be_data = item.get('to_be_process', {}) # New To-Be Data
        to_be_steps = to_be_data.get('steps', [])
        to_be_narrative = to_be_data.get('narrative', 'Process automation via Service Bus.')
        ...
        bpm_diagram = get_mermaid_bpmn(as_is_steps)
        ...
        with open(md_file, 'w', encoding='utf-8') as f:
            ...
            # To-Be Section (Detailed)
            f.write("## Future State Process (TO-BE)\n")
            f.write(f"### Narrative\n{to_be_narrative}\n\n")
            f.write("### Optimized Steps (Digital)\n| Step | Actor | Action | System |\n|---|---|---|---|\n")
            for step in to_be_steps:
                f.write(f"| {step.get('step_number')} | {step.get('actor')} | {step.get('description')} | {step.get('system')} |\n")
            f.write("\n---\n\n")
    ...
    print(f"Successfully generated {count} Final BPM documents.")
