# Rongo University – Student Admission

## Cover Page
- **Ministry/Department/Agency (MDA):** Rongo University
...
### Legal Context
- The Rongo University Act (Inferred).
...
### Scope
- **In Scope:** End-to-end processing within Rongo University.
...
graph TD
    Start((Start)) --> S1
    subgraph Student [Student]
        S1["Student accepts admission via Portal."]
        S3["Student pays fees via integrated Payment Gateway."]
    end
    subgraph System [System]
        S2["System pulls KCSE results from KNEC via Service Bu..."]
        S4["Smart Student ID is activated for access to Librar..."]
    end
    S1 --> S2
    S2 --> S3
    S3 --> S4
...
### Narrative
The To-Be process creates a unified Student Digital Profile, integrating admissions, library, and finance into a single view.

### Optimized Steps (Digital)
| Step | Actor | Action | System |
|---|---|---|---|
| 1 | Student | Student accepts admission via Portal. | ERP |
| 2 | System | System pulls KCSE results from KNEC via Service Bus for verification. | Service Bus / KNEC |
| 3 | Student | Student pays fees via integrated Payment Gateway. | Payment Gateway |
| 4 | System | Smart Student ID is activated for access to Library and Hostels. | Access Control |
