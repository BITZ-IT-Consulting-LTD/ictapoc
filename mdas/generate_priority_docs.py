...
output_dir = '/Users/mac/ictapoc/mdas/docs_priority'
zip_filename = '/Users/mac/ictapoc/mdas/mda_bpm_docs_priority.zip'
...
try:
    ...
    print(f"Generating Priority-Included Documentation for {len(items)} MDAs...")
    ...
    for item in items:
        ...
        elif "refugee" in narrative.lower(): process_name = "Refugee_Registration"
        elif "birth" in narrative.lower(): process_name = "Birth_Registration"
        elif "hustler" in narrative.lower(): process_name = "Hustler_Fund_Application"
        ...
        elif "research" in narrative.lower(): process_name = "Research_Authorization"
        elif "procurement" in narrative.lower() or "tender" in narrative.lower(): process_name = "Procurement_Process"
        ...
        mandate_text = proc_overview.get('process_objective', 'To deliver public services effectively.')
        legal_context_list = proc_overview.get('policy_legal_context', [])
        if legal_context_list:
            legal_context = "\n".join([f"- {l}" for l in legal_context_list])
        else:
            legal_context = f"- The {mda_name} Act (Inferred)."

        metadata = item.get('metadata', {})
        sources = metadata.get('source_urls', [])
        priority_tag = metadata.get('priority_status', '')
        ...
        with open(md_file, 'w', encoding='utf-8') as f:
            ...
            f.write("## References & Evidence\n")
            f.write("The information in this document was derived from the following official sources:\n\n")
            if sources:
                for url in sources: f.write(f"- [{url}]({url})\n")
            else:
                f.write("- Official Mandate and Service Charter (Inferred from Statutory Acts).\n")

        count += 1

    print(f"Successfully generated {count} Priority-Updated BPM documents.")
