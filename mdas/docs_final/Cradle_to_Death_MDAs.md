# Cradle to Death: Government Service Delivery Lifecycle

This document outlines the key Ministries, Departments, and Agencies (MDAs) involved in a citizen's lifecycle, from birth registration ("Cradle") to succession and estate administration ("Death").

---

## 1. The Cradle (Birth & Identity)
**Objective:** Establish legal existence and foundational identity.

*   **Civil Registration Services (CRS)**
    *   *File:* [`_CIVIL_REGISTRATION_SERVICES_CRS____Service_Delivery.md`](./mdas/docs_final/_CIVIL_REGISTRATION_SERVICES_CRS____Service_Delivery.md)
    *   *Role:* Issuance of **Birth Certificates** and **Death Certificates**. The primary source of civil status data.
*   **Ministry of Health**
    *   *File:* `Ministry_of_Health___Service_Delivery.md`
    *   *Role:* Notification of birth occurrences from health facilities (e.g., vaccination records).
*   **Integrated Population Registration System (IPRS)**
    *   *File:* `_INTEGRATED_POPULATION_REGISTRATION_SYSTEM___Service_Delivery.md`
    *   *Role:* The "Single Source of Truth" that links the birth record (unique identifier) to all subsequent government services.
*   **National Health Insurance Fund (NHIF) / Social Health Authority (SHA)**
    *   *File:* [`National_Health_Insurance_Fund___Service_Delivery.md`](./mdas/docs_final/National_Health_Insurance_Fund___Service_Delivery.md)
    *   *Role:* Mandatory health insurance coverage (Linda Mama for birth).

## 2. Childhood & Education
**Objective:** Facilitate learning, development, and academic progression.

*   **Ministry of Education**
    *   *File:* [`Ministry_of_Education___Service_Delivery.md`](./mdas/docs_final/Ministry_of_Education___Service_Delivery.md)
    *   *Role:* issuance of **NEMIS** (National Education Management Information System) numbers for school enrollment.
*   **Kenya National Examinations Council (KNEC)**
    *   *File:* `Kenya_National_Examinations_Council___Service_Delivery.md`
    *   *Role:* Administration of national exams (KPSEA, KCSE) and certification.
*   **Kenya Universities and Colleges Central Placement Service (KUCCPS)**
    *   *File:* `Kenya_Universities_And_Colleges_Central_Placement_Service___Student_Admission.md`
    *   *Role:* Placement of students into universities and colleges.
*   **Higher Education Loans Board (HELB)**
    *   *File:* `Higher_Education_Loans_Board___Loan_Processing.md`
    *   *Role:* Funding for higher education (loans and bursaries).

## 3. Coming of Age (Adulthood & Citizenship)
**Objective:** Transition to legal adulthood, rights, and responsibilities.

*   **National Registration Bureau (NRB)**
    *   *File:* [`_NATIONAL_REGISTRATION_BUREAU_NRB____Service_Delivery.md`](./mdas/docs_final/_NATIONAL_REGISTRATION_BUREAU_NRB____Service_Delivery.md)
    *   *Role:* Issuance of the **National Identity Card (ID)** at age 18.
*   **Immigration & Citizen Services**
    *   *File:* [`STATE_DEPARTMENT_FOR_IMMIGRATION_AND_CITIZEN_SERVICES___Passport_Application.md`](./mdas/docs_final/STATE_DEPARTMENT_FOR_IMMIGRATION_AND_CITIZEN_SERVICES___Passport_Application.md)
    *   *Role:* Issuance of **Passports** and travel documents.
*   **National Transport and Safety Authority (NTSA)**
    *   *File:* `NATIONAL_TRANSPORT_AND_SAFETY_AUTHORITY_NTSA____Driving_License_Renewal.md`
    *   *Role:* Driver training, testing, and **Driving License** issuance.

## 4. Economic Life (Employment & Business)
**Objective:** Participation in the economy, taxation, and social security.

*   **Kenya Revenue Authority (KRA)**
    *   *File:* [`KENYA_REVENUE_AUTHORITY_KRA____Tax_Return_Filing.md`](./mdas/docs_final/KENYA_REVENUE_AUTHORITY_KRA____Tax_Return_Filing.md)
    *   *Role:* **KRA PIN** registration and tax compliance.
*   **Business Registration Service (BRS)**
    *   *File:* [`Business_Registration_Service___Service_Delivery.md`](./mdas/docs_final/Business_Registration_Service___Service_Delivery.md)
    *   *Role:* Registration of Business Names, Companies, and Partnerships.
*   **National Social Security Fund (NSSF)**
    *   *File:* `National_Social_Security_Fund___Service_Delivery.md`
    *   *Role:* Mandatory retirement savings registration.
*   **National Health Insurance Fund (NHIF) / Social Health Authority (SHA)**
    *   *File:* [`National_Health_Insurance_Fund___Service_Delivery.md`](./mdas/docs_final/National_Health_Insurance_Fund___Service_Delivery.md)
    *   *Role:* Mandatory health insurance coverage.

## 5. Family & Property
**Objective:** Managing assets, family units, and legal affairs.

*   **State Law Office (Attorney General)**
    *   *File:* [`State_Law_Office_The___Service_Delivery.md`](./mdas/docs_final/State_Law_Office_The___Service_Delivery.md)
    *   *Role:* **Marriage Registration**, Societies registration, and legal advice.
*   **Ministry of Lands**
    *   *File:* [`STATE_DEPARTMENT_FOR_LANDS_AND_PHYSICAL_PLANNING___Service_Delivery.md`](./mdas/docs_final/STATE_DEPARTMENT_FOR_LANDS_AND_PHYSICAL_PLANNING___Service_Delivery.md)
    *   *Role:* Land registration, title deeds, and property transfers (Ardhisasa).

## 6. The Death (End of Life & Succession)
**Objective:** Closing the lifecycle, succession, and asset distribution.

*   **Civil Registration Services (CRS)**
    *   *File:* [`_CIVIL_REGISTRATION_SERVICES_CRS____Service_Delivery.md`](./mdas/docs_final/_CIVIL_REGISTRATION_SERVICES_CRS____Service_Delivery.md)
    *   *Role:* Issuance of **Death Certificates** and Burial Permits.
*   **The Judiciary**
    *   *File:* [`THE_JUDICIARY___Service_Delivery.md`](./mdas/docs_final/THE_JUDICIARY___Service_Delivery.md)
    *   *Role:* **Succession Causes**, probate administration, and wills.
*   **Unclaimed Financial Assets Authority (UFAA)**
    *   *File:* `Unclaimed_Financial_Assets_Authority___Service_Delivery.md`
    *   *Role:* Reunification of unclaimed assets (bank accounts, shares) with beneficiaries.
*   **Public Trustee (under State Law Office)**
    *   *File:* `OFFICE_OF_THE_ATTORNEY_GENERAL_AG____Service_Delivery.md`
    *   *Role:* Administration of estates for deceased persons without a will or capable administrator.
